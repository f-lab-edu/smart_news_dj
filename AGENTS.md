...
- 가변 집합은 `mutableSetOf()` 사용 (삽입 순서가 필요해도 JVM Kotlin에서 동일한 의미로 쓴다)
- 순서 보장이 필요 없는 경우 가장 단순한 컬렉션 선택
- 자료구조 선택이 의도를 드러내도록 작성

### Avoid
- 순서 보장이 필요 없는데 `linkedMapOf()` 사용 금지
- `LinkedHashSet` 직접 생성 금지 (`mutableSetOf()`로 통일)
- 불필요하게 의미를 가지는 컬렉션 선택 금지

### Rationale
- `linkedMapOf()`는 삽입 순서 보장이라는 의미를 가진다
- 실제 요구사항이 없는데 사용하면 잘못된 의도를 암시한다
- JVM Kotlin에서 `mutableSetOf()`는 삽입 순서를 유지하는 집합이므로 `LinkedHashSet` 직접 생성과 역할이 겹친다
